...
Returns a `builtin.func` operation corresponding to the ircode of the provided method.
...
function code_mlir(f, types; ctx=Context())
    ...
    for (block_id, (b, bb)) in enumerate(zip(blocks, ir.cfg.blocks))
        ...
        for sidx in bb.stmts
            ...
            elseif inst isa GotoNode
                ...
                push!(current_block, std.br(ctx, dest, args; loc=Location(ctx, line)))
            elseif inst isa GotoIfNot
                ...
                cond_br = std.cond_br(ctx, cond, other_dest, dest, true_args, false_args; loc=Location(ctx, line))
                push!(current_block, cond_br)
            elseif inst isa ReturnNode
                line = ir.linetable[stmt[:line]]
                push!(current_block, std.return_(ctx, [get_value(inst.val)]; loc=Location(ctx, line)))
            else
    ...
    state = OperationState("builtin.func", Location(ctx))
    ...
    IR.add_attributes!(state, [
        ...
        NamedAttribute(ctx, "type", IR.Attribute(ftype)),
    ])
...
opm = IR.OpPassManager(pm, "builtin.func")
...
MLIR.API.mlirPassManagerAddOwnedPass(pm, MLIR.API.mlirCreateConversionConvertStandardToLLVM())
