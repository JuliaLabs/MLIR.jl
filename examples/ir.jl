...
function make_and_dump_add(ctx, loc)
    ...
    func_attrs = [IR.NamedAttribute("type", func_type_attr), IR.NamedAttribute("sym_name", func_name_attr)]
    ...
    index_zero_value_attr = IR.NamedAttribute("value", index_zero_literal)
    ...
    loop_body = IR.create_block(index_type)
    IR.push!(loop_body_region, loop_body)
    index_one_literal = IR.parse_attribute(ctx, "1: index")
    index_one_value_attr = IR.NamedAttribute("value", index_one_literal)
