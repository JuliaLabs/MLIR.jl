...
function populate_loop_body(ctx, loop_body, loc, func_body)
    ...
    IR.push_operands!(load_lhs_state, load_lhs_operands)
    IR.push_results!(load_lhs_state, f32)
    ...
    IR.push_operands!(load_rhs_state, load_rhs_operands)
    IR.push_results!(load_rhs_state, f32)
    ...
    IR.push_operands!(add_state, add_operands)
    IR.push_results!(add_state, f32)
    ...
    IR.push_operands!(store_state, store_operands)
...
function make_and_dump_add(ctx, loc)
    ...
    IR.push_attributes!(func_state, func_attrs)
    IR.push_regions!(func_state, func_region)
    ...
    IR.push_results!(const_zero_state, index_type)
    IR.push_attributes!(const_zero_state, index_zero_value_attr)
    ...
    IR.push_operands!(dim_state, dim_operands)
    IR.push_results!(dim_state, index_type)
    ...
    IR.push_results!(const_one_state, index_type)
    IR.push_attributes!(const_one_state, index_one_value_attr)
    ...
    IR.push_operands!(loop_state, loop_operands)
    IR.push_regions!(loop_state, loop_body_region)
