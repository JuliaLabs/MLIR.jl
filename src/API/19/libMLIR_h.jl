...
function mlirSetGlobalDebugTypes(types, n)
    @ccall (MLIR_C_PATH[]).mlirSetGlobalDebugTypes(types::Ptr{Cstring}, n::intptr_t)::Cvoid
end
...
function mlirLLVMStructTypeGetNumElementTypes(type)
    @ccall (MLIR_C_PATH[]).mlirLLVMStructTypeGetNumElementTypes(type::MlirType)::intptr_t
end
...
const MLIR_ENABLE_ROCM_CONVERSIONS = 0
