...
```
"""
function apply_constraint(args; name, constParams=nothing, trueDest::Block, falseDest::Block, location=Location())
    results = IR.Type[]
...
```
"""
function apply_rewrite(args; results_::Vector{IR.Type}, name, constParams=nothing, location=Location())
...
```
"""
function check_operand_count(operation; count, compareAtLeast=nothing, trueDest::Block, falseDest::Block, location=Location())
    results = IR.Type[]
...
```
"""
function check_operation_name(operation; name, trueDest::Block, falseDest::Block, location=Location())
    results = IR.Type[]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[trueDest, falseDest, ]
...
```
"""
function check_result_count(operation; count, compareAtLeast=nothing, trueDest::Block, falseDest::Block, location=Location())
    results = IR.Type[]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[trueDest, falseDest, ]
...
```
"""
function create_operation(operands_, attributes_, types; operation::IR.Type, name, attributeNames, location=Location())
...
```
"""
function get_attribute(operation; attribute::IR.Type, name, location=Location())
...
```
"""
function get_defining_op(value; operation::IR.Type, location=Location())
    results = IR.Type[operation, ]
    operands = Value[value(value), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function get_operand(operation; value::IR.Type, index, location=Location())
...
```
"""
function get_operands(operation; value::IR.Type, index=nothing, location=Location())
    results = IR.Type[value, ]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function get_result(operation; value::IR.Type, index, location=Location())
    results = IR.Type[value, ]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function get_results(operation; value::IR.Type, index=nothing, location=Location())
    results = IR.Type[value, ]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function replace(operation, replValues; location=Location())
    results = IR.Type[]
...
```
"""
function switch_operand_count(operation; caseValues, defaultDest::Block, cases::Vector{Block}, location=Location())
    results = IR.Type[]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[defaultDest, cases..., ]
...
```
"""
function switch_operation_name(operation; caseValues, defaultDest::Block, cases::Vector{Block}, location=Location())
    results = IR.Type[]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[defaultDest, cases..., ]
...
```
"""
function switch_result_count(operation; caseValues, defaultDest::Block, cases::Vector{Block}, location=Location())
    results = IR.Type[]
    operands = Value[value(operation), ]
    owned_regions = Region[]
    successors = Block[defaultDest, cases..., ]
