...
```
"""
function assuming_all(inputs::Vector{Value}; result::MLIRType, location=Location())
    results = MLIRType[result, ]
    operands = Value[inputs..., ]
    owned_regions = Region[]
    successors = Block[]
...
concat([], [4,5,6]) -> [4,5,6]
"""
function concat(lhs::Value, rhs::Value; result=nothing::Union{Nothing, MLIRType}, location=Location())
    results = MLIRType[]
    operands = Value[lhs, rhs, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function from_extents(extents::Vector{Value}; shape::MLIRType, location=Location())
    results = MLIRType[shape, ]
