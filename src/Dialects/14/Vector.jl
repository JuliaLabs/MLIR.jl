...
```
"""
function extract_map(vector, ids; result_0::IR.Type, location=Location())
    results = IR.Type[result_0, ]
    operands = Value[value(vector), value.(ids)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function extract(vector; result_0::IR.Type, position, location=Location())
    results = IR.Type[result_0, ]
    operands = Value[value(vector), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function fma(lhs, rhs, acc; result::IR.Type, location=Location())
    results = IR.Type[result, ]
...
```
"""
function insertelement(source, dest, position=nothing; result::IR.Type, location=Location())
    results = IR.Type[result, ]
...
```
"""
function insert_map(vector, dest, ids; result::IR.Type, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(vector), value(dest), value.(ids)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function insert(source, dest; res::IR.Type, position, location=Location())
    results = IR.Type[res, ]
    operands = Value[value(source), value(dest), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function insert_strided_slice(source, dest; res::IR.Type, offsets, strides, location=Location())
    results = IR.Type[res, ]
    operands = Value[value(source), value(dest), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function multi_reduction(source; dest::IR.Type, kind, reduction_dims, location=Location())
...
```
"""
function reduction(vector, acc; dest::IR.Type, kind, location=Location())
    results = IR.Type[dest, ]
    operands = Value[value(vector), value.(acc)..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function scan(source, initial_value; dest::IR.Type, accumulated_value::IR.Type, kind, reduction_dim, inclusive, location=Location())
    results = IR.Type[dest, accumulated_value, ]
...
```
"""
function shuffle(v1, v2; vector::IR.Type, mask, location=Location())
    results = IR.Type[vector, ]
