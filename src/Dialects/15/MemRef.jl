...
```
"""
function atomic_rmw(value, memref, indices; result::MLIRType, kind, location=Location())
...
```
"""
function dim(source, index; result::MLIRType, location=Location())
    results = MLIRType[result, ]
...
```
"""
function rank(memref; result_0::MLIRType, location=Location())
...
function tensor_store(tensor, memref; location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(tensor), get_value(memref), ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
