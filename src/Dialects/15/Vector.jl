...
```
"""
function extract_map(vector, ids; result_0::MLIRType, location=Location())
    results = MLIRType[result_0, ]
    operands = API.MlirValue[get_value(vector), get_value.(ids)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function insert_map(vector, dest, ids; result=nothing::Union{Nothing, MLIRType}, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(vector), get_value(dest), get_value.(ids)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    (result != nothing) && push!(results, result)
