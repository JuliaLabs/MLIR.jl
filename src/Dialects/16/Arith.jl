...
```
"""
function maxf(lhs, rhs; result=nothing::Union{Nothing, MLIRType}, fastmath=nothing, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function minf(lhs, rhs; result=nothing::Union{Nothing, MLIRType}, fastmath=nothing, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(lhs), get_value(rhs), ]
    owned_regions = Region[]
    successors = Block[]
