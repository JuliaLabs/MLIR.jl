...
```
"""
function foreach_thread(num_threads, outputs; results::Vector{MLIRType}, mapping=nothing, region::Region, location=Location())
    results = MLIRType[results..., ]
    operands = API.MlirValue[get_value.(num_threads)..., get_value.(outputs)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function index_switch(arg; results::Vector{MLIRType}, cases, defaultRegion::Region, caseRegions::Vector{Region}, location=Location())
    results = MLIRType[results..., ]
