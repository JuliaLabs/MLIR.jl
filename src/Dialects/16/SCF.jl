...
```
"""
function foreach_thread(num_threads, outputs; results_::Vector{IR.Type}, mapping=nothing, region::Region, location=Location())
    results = IR.Type[results_..., ]
    operands = Value[value.(num_threads)..., value.(outputs)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
