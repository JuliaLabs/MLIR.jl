...
```
"""
function foreach_thread(num_threads::Vector{Value}, outputs::Vector{Value}; results::Vector{MLIRType}, mapping=nothing, region::Region, location=Location())
    results = MLIRType[results..., ]
...
```
"""
function index_switch(arg::Value; results::Vector{MLIRType}, cases, defaultRegion::Region, caseRegions::Vector{Region}, location=Location())
    results = MLIRType[results..., ]
    operands = Value[arg, ]
