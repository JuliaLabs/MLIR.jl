...
```
"""
function compress(values, filled, added, count, tensor, indices; result=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = Value[value(values), value(filled), value(added), value(count), value(tensor), value.(indices)..., ]
...
```
"""
function foreach(tensor, initArgs; results_::Vector{IR.Type}, region::Region, location=Location())
...
```
"""
function storage_specifier_get(specifier; result::IR.Type, specifierKind, dim=nothing, location=Location())
    results = IR.Type[result, ]
...
```
"""
function insert(value, tensor, indices; result=nothing::Union{Nothing, IR.Type}, location=Location())
    results = IR.Type[]
    operands = Value[value(value), value(tensor), value.(indices)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(result) && push!(results, result)
...
```
"""
function new(source; result::IR.Type, expandSymmetry=nothing, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(source), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function storage_specifier_set(specifier, value; result=nothing::Union{Nothing, IR.Type}, specifierKind, dim=nothing, location=Location())
    results = IR.Type[]
...
```
"""
function indices_buffer(tensor; result::IR.Type, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(tensor), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function indices(tensor; result::IR.Type, dimension, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(tensor), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function pointers(tensor; result::IR.Type, dimension, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(tensor), ]
    owned_regions = Region[]
    successors = Block[]
