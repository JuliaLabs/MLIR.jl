...
function empty(dynamicSizes; result::IR.Type, location=Location())
    results = IR.Type[result, ]
    operands = Value[value.(dynamicSizes)..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
function gather(source, indices; result::IR.Type, gather_dims, unique=nothing, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(source), value(indices), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function pack(source, dest, padding_value=nothing; inner_tiles, result::IR.Type, outer_dims_perm=nothing, inner_dims_pos, static_inner_tiles, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(source), value(dest), value.(inner_tiles)..., ]
    owned_regions = Region[]
    successors = Block[]
    ...
    !isnothing(padding_value) && push!(operands, value(padding_value))
...
function scatter(source, dest, indices; result::IR.Type, scatter_dims, unique=nothing, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(source), value(dest), value(indices), ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function unpack(source, dest, inner_tiles; result::IR.Type, outer_dims_perm=nothing, inner_dims_pos, static_inner_tiles, location=Location())
    results = IR.Type[result, ]
    operands = Value[value(source), value(dest), value.(inner_tiles)..., ]
    owned_regions = Region[]
    successors = Block[]
