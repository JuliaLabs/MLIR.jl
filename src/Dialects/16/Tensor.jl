...
"""
function empty(dynamicSizes; result::MLIRType, location=Location())
    results = MLIRType[result, ]
...
```
"""
function gather(source, indices; result::MLIRType, gather_dims, unique=nothing, location=Location())
    results = MLIRType[result, ]
...
```
"""
function pack(source, dest, padding_value=nothing; inner_tiles, result::MLIRType, outer_dims_perm=nothing, inner_dims_pos, static_inner_tiles, location=Location())
    results = MLIRType[result, ]
    ...
    (padding_value != nothing) && push!(operands, get_valuepadding_value)
...
```
"""
function scatter(source, dest, indices; result::MLIRType, scatter_dims, unique=nothing, location=Location())
    results = MLIRType[result, ]
...
```
"""
function unpack(source, dest, inner_tiles; result::MLIRType, outer_dims_perm=nothing, inner_dims_pos, static_inner_tiles, location=Location())
    results = MLIRType[result, ]
    operands = API.MlirValue[get_value(source), get_value(dest), get_value.(inner_tiles)..., ]
    owned_regions = Region[]
    successors = Block[]
