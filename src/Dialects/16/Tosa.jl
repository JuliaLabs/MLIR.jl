...
is backend specific.
"""
function custom(inputs; outputs::Vector{MLIRType}, identifier, config, implementation_attrs, location=Location())
...
"""
function equal(input1, input2; output=nothing::Union{Nothing, MLIRType}, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(input1), get_value(input2), ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function logical_not(input1; output=nothing::Union{Nothing, MLIRType}, location=Location())
    results = MLIRType[]
    operands = API.MlirValue[get_value(input1), ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function rfft2d(input; output_real::MLIRType, output_imag::MLIRType, location=Location())
...
"""
function resize(input; output::MLIRType, scale, offset, border, mode, location=Location())
    results = MLIRType[output, ]
    operands = API.MlirValue[get_value(input), ]
    owned_regions = Region[]
    successors = Block[]
