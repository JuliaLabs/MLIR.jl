...
function simdloop(lowerBound, upperBound, step, aligned_vars, if_expr=nothing; nontemporal_vars, alignment_values=nothing, order_val=nothing, simdlen=nothing, safelen=nothing, inclusive=nothing, region::Region, location=Location())
    results = IR.Type[]
    operands = Value[value.(lowerBound)..., value.(upperBound)..., value.(step)..., value.(aligned_vars)..., value.(nontemporal_vars)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(if_expr) && push!(operands, value(if_expr))
...
"""
function target(if_expr=nothing; device=nothing, thread_limit=nothing, map_operands, nowait=nothing, map_types=nothing, region::Region, location=Location())
    results = IR.Type[]
    operands = Value[value.(map_operands)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(if_expr) && push!(operands, value(if_expr))
    ...
    push!(attributes, operandsegmentsizes([(if_expr==nothing) ? 0 : 1(device==nothing) ? 0 : 1(thread_limit==nothing) ? 0 : 1length(map_operands), ]))
    !isnothing(nowait) && push!(attributes, namedattribute("nowait", nowait))
...
function target_data(if_expr=nothing; device=nothing, use_device_ptr, use_device_addr, map_operands, map_types, region::Region, location=Location())
    results = IR.Type[]
    operands = Value[value.(use_device_ptr)..., value.(use_device_addr)..., value.(map_operands)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
function task(if_expr=nothing; final_expr=nothing, in_reduction_vars, priority=nothing, depend_vars, allocate_vars, allocators_vars, untied=nothing, mergeable=nothing, in_reductions=nothing, depends=nothing, region::Region, location=Location())
    results = IR.Type[]
    operands = Value[value.(in_reduction_vars)..., value.(depend_vars)..., value.(allocate_vars)..., value.(allocators_vars)..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(if_expr) && push!(operands, value(if_expr))
...
function teams(num_teams_lower=nothing; num_teams_upper=nothing, if_expr=nothing, thread_limit=nothing, allocate_vars, allocators_vars, reduction_vars, reductions=nothing, region::Region, location=Location())
    ...
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(num_teams_lower) && push!(operands, value(num_teams_lower))
    !isnothing(num_teams_upper) && push!(operands, value(num_teams_upper))
    !isnothing(if_expr) && push!(operands, value(if_expr))
    !isnothing(thread_limit) && push!(operands, value(thread_limit))
