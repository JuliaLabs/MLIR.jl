...
function bounds(
    ...
    push!(attributes, operandsegmentsizes([
        if (lower_bound == nothing)
            0
        elseif 1(upper_bound == nothing)
            0
        elseif 1(extent == nothing)
            0
        elseif 1(stride == nothing)
            0
        elseif 1(start_idx == nothing)
            0
        else
            1
        end,
    ...
    !isnothing(stride_in_bytes) &&
        push!(attributes, namedattribute("stride_in_bytes", stride_in_bytes))
...
function map_info(
    ...
    push!(
        attributes, operandsegmentsizes([1, (var_ptr_ptr == nothing) ? 0 : 1length(bounds)])
    )
    !isnothing(map_type) && push!(attributes, namedattribute("map_type", map_type))
    !isnothing(map_capture_type) &&
        push!(attributes, namedattribute("map_capture_type", map_capture_type))
    !isnothing(name) && push!(attributes, namedattribute("name", name))
...
function simdloop(
    ...
    !isnothing(alignment_values) &&
        push!(attributes, namedattribute("alignment_values", alignment_values))
