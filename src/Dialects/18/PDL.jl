...
```
"""
function apply_native_constraint(
    args::Vector{Value}; name, isNegated=nothing, location=Location()
)
    _results = IR.Type[]
    ...
    !isnothing(isNegated) && push!(_attributes, namedattribute("isNegated", isNegated))
...
"""
function attribute(
    valueType=nothing::Union{Nothing,Value};
    attr::IR.Type,
    value=nothing,
    location=Location(),
)
...
"""
function operation(
    operandValues::Vector{Value},
    attributeValues::Vector{Value},
    typeValues::Vector{Value};
    op::IR.Type,
    opName=nothing,
    attributeValueNames,
    location=Location(),
    ...
    _results = IR.Type[op,]
    _operands = Value[operandValues..., attributeValues..., typeValues...]
    _owned_regions = Region[]
    _successors = Block[]
...
function replace(
    ...
    _results = IR.Type[]
    _operands = Value[opValue, replValues...]
    _owned_regions = Region[]
    _successors = Block[]
    _attributes = NamedAttribute[]
