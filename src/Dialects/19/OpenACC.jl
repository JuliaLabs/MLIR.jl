...
function attach(
    ...
    bounds::Vector{Value},
    asyncOperands::Vector{Value},
    accPtr::IR.Type,
    asyncOperandsDeviceType=nothing,
    asyncOnly=nothing,
    dataClause=nothing,
    structured=nothing,
    implicit=nothing,
    ...
    _operands = Value[varPtr, bounds..., asyncOperands...]
    _owned_regions = Region[]
    _successors = Block[]
    _attributes = NamedAttribute[]
    ...
    push!(
        _attributes,
        operandsegmentsizes([
            1, isnothing(varPtrPtr) ? 0 : 1, length(bounds), length(asyncOperands)
        ]),
    )
    !isnothing(asyncOperandsDeviceType) && push!(
        _attributes, namedattribute("asyncOperandsDeviceType", asyncOperandsDeviceType)
    )
...
function copyout(
    ...
    _results = IR.Type[]
    _operands = Value[accPtr, varPtr, bounds..., asyncOperands...]
    _owned_regions = Region[]
    _successors = Block[]
    _attributes = NamedAttribute[]
    push!(_attributes, operandsegmentsizes([1, 1, length(bounds), length(asyncOperands)]))
    !isnothing(asyncOperandsDeviceType) && push!(
        _attributes, namedattribute("asyncOperandsDeviceType", asyncOperandsDeviceType)
    )
    !isnothing(asyncOnly) && push!(_attributes, namedattribute("asyncOnly", asyncOnly))
...
function data(
    ...
    _results = IR.Type[]
    _operands = Value[asyncOperands..., waitOperands..., dataClauseOperands...]
    _owned_regions = Region[region,]
    _successors = Block[]
    _attributes = NamedAttribute[]
    ...
    !isnothing(hasWaitDevnum) &&
        push!(_attributes, namedattribute("hasWaitDevnum", hasWaitDevnum))
...
function delete(
    ...
    _results = IR.Type[]
    _operands = Value[accPtr, bounds..., asyncOperands...]
    _owned_regions = Region[]
    _successors = Block[]
    _attributes = NamedAttribute[]
    push!(_attributes, operandsegmentsizes([1, length(bounds), length(asyncOperands)]))
    !isnothing(asyncOperandsDeviceType) && push!(
        _attributes, namedattribute("asyncOperandsDeviceType", asyncOperandsDeviceType)
    )
    !isnothing(asyncOnly) && push!(_attributes, namedattribute("asyncOnly", asyncOnly))
...
function kernels(
    ...
    !isnothing(defaultAttr) &&
        push!(_attributes, namedattribute("defaultAttr", defaultAttr))
    !isnothing(combined) && push!(_attributes, namedattribute("combined", combined))
