...
"""
IntegerSet(ndims, nsymbols; context::Context=context()) = IntegerSet(API.Dispatcher.mlirIntegerSetEmptyGet(context, ndims, nsymbols))
...
"""
IntegerSet(ndims, nsymbols, constraints, eqflags; context::Context=context()) = IntegerSet(API.Dispatcher.mlirIntegerSetGet(context, ndims, nsymbols, length(constraints), pointer(constraints), pointer(eqflags)))
...
"""
Base.replace(set::IntegerSet, dim_replacements, symbol_replacements) = IntegerSet(API.Dispatcher.mlirIntegerSetReplaceGet(set, dim_replacements, symbol_replacements, length(dim_replacements), length(symbol_replacements)))
...
"""
Base.:(==)(a::IntegerSet, b::IntegerSet) = API.Dispatcher.mlirIntegerSetEqual(a, b)
...
"""
context(set::IntegerSet) = Context(API.Dispatcher.mlirIntegerSetGetContext(set.set))
...
"""
isempty(set::IntegerSet) = API.Dispatcher.mlirIntegerSetIsCanonicalEmpty(set)
...
"""
Base.ndims(set::IntegerSet) = API.Dispatcher.mlirIntegerSetGetNumDims(set)
...
"""
nsymbols(set::IntegerSet) = API.Dispatcher.mlirIntegerSetGetNumSymbols(set)
...
"""
ninputs(set::IntegerSet) = API.Dispatcher.mlirIntegerSetGetNumInputs(set)
...
"""
nconstraints(set::IntegerSet) = API.Dispatcher.mlirIntegerSetGetNumConstraints(set)
...
"""
nequalities(set::IntegerSet) = API.Dispatcher.mlirIntegerSetGetNumEqualities(set)
...
"""
ninequalities(set::IntegerSet) = API.Dispatcher.mlirIntegerSetGetNumInequalities(set)
...
"""
constraint(set::IntegerSet, i) = API.Dispatcher.mlirIntegerSetGetConstraint(set, i)
...
"""
isconstrainteq(set::IntegerSet, i) = API.Dispatcher.mlirIntegerSetIsConstraintEq(set, i)

function Base.show(io::IO, set::IntegerSet)
    print(io, "IntegerSet(#= ")
    c_print_callback = @cfunction(print_callback, Cvoid, (API.MlirStringRef, Any))
    ref = Ref(io)
    API.Dispatcher.mlirIntegerSetPrint(set, c_print_callback, ref)
    print(io, " =#)")
end
